...
11. **The `jelou test` synthetic tester is not a real WhatsApp contact — don't trust it for anything that depends on a resolved identity.** Its `$user` object is missing fields a real conversation has (confirmed: `phone`, via `$user.get("phone")` in `references/patterns.md` — treat as a warning that *other* `$user.*` fields could have the same gap, not a closed list). A probe that fails there doesn't prove a real customer would hit the same failure; only trust a synthetic-tester result as evidence when you have a reason to believe the field it touches is one the harness actually populates (the `{{$user.phone}}` *template* form, or plain `$user.get("id")`, are known-good so far). When in doubt, say in the report that a claim is based on real-conversation evidence, synthetic-tester evidence, or code-reading alone — don't blur the three together.
12. **This entire skill only applies to the WhatsApp channel.** `$user.phone`, the BSUID-unresolved-identity problem, and `contact_info_request` are WhatsApp-specific platform concepts — a Web session, a Facebook PSID, or an Instagram-scoped id is never a phone number and never goes through this resolution path. If the company has no WhatsApp channel at all, say so plainly and stop — don't run the rest of the phases for a company this skill has nothing to offer. If the company has WhatsApp *and* other channels (common — many bots also run on Web/Facebook/Instagram), scope every phase from Phase 3 onward to WhatsApp-channel workflows only (the `.whatsapp.json` filename suffix is the source of truth, per `jelou-graph`'s own convention) and exclude every other channel's workflows the same transparent way Phase 3 excludes stub workflows — never silently, always logged in the report.
...
- **Which workflows are WhatsApp at all (Rule 12).** Every pulled workflow file is named `<slug>.<channel>.json` — anything that isn't `.whatsapp.json` (`.web.json`, `.facebook.json`, `.instagram.json`, etc.) is out of scope for this entire skill and gets excluded from Phase 5 onward, the same transparent way as the stub-workflow exclusion right below: keep the list (name + channel) and put it in the report so the person can see what was skipped and why, never drop it silently.
- Which workflow(s) have `default: true` (there should be exactly one) **among the WhatsApp-channel workflows** — a non-WhatsApp workflow being the project's overall default doesn't change anything here.
...
`jelou-graph`'s relationship commands (`summary`/`references`/`focus`/`paths`/`impact`) only model structural relations (`CALLS`/`FLOWS_TO`/`CONTAINS`/`ROUTES_TO`/`READS`/`WRITES`) and can't be queried with a variable name like `$user.id`. **But `jelou graph search --text` is a separate, verified capability** — a literal full-text search across every locally pulled node's own configuration, confirmed to reach `CODE` node `content`, `AI_TASK` `instructions`, and `CONDITIONAL` term values (not just titles/labels), plus it tells you per-node reachability for free. That's what Phase 5 leads with now — see `references/patterns.md` ("What `jelou graph search --text` actually does") for the full evidence and its one real gap (tool-backing workflows).

## Phase 4 — Provider + real routing-model check (empirical, not assumed)

1. `jelou channels list --include-sandbox --agent`, filter to this `projectId`. Read `provider`. **If there is no WhatsApp channel at all (Rule 12), this skill doesn't apply to this company — say so plainly and stop here, don't run Phases 5 onward for nothing.** If there are multiple WhatsApp channels with different providers, that's a genuine ambiguity: stop and flag it in the report as an open question rather than guessing which one governs the guard design. (Having WhatsApp *plus* other non-WhatsApp channels is normal and not a blocker — Phase 3 already scoped the workflow set to WhatsApp only.)
...
Lead with `jelou graph search --text`, one call per literal pattern (see `references/patterns.md` for the exact list — this is literal-string matching, not regex, so search **both quote styles** separately, e.g. `user.get("id")` and `user.get('id')`):

```
jelou graph search --text '<pattern>' --limit 500 --agent
```

For every call:
- **Check `truncated` every time.** If `true`, re-run with `--limit` raised to at least the returned `matched` count before moving on — never treat a truncated result as the complete picture.
- **Cross-check `reachable` against Phase 3's stub-exclusion list.** A hit inside a workflow Phase 3 flagged as a stub (nothing connected to `START`) coming back `reachable: true` — or the reverse — is a contradiction: stop and re-verify rather than trusting either source blindly.
- **Open the `source` file and read the whole node's configuration for every hit** before classifying — the search result only gives you the node's id/label/type/reachability, not the matching text in its surrounding context.

**What this does not cover — Phase 6.5 stays mandatory.** `jelou graph search --text` only scans files already part of the local, pulled graph (the same set `jelou pull` brought down in Phase 2). It does **not** reach into a tool's own separate backing workflow (`availability: referenced`, not `local`) — that gap is exactly why Phase 6.5's own pull-and-search of every tool's workflow still has to happen, unchanged.

**First run against a new company — a quick sanity check, not a routine step.** This method was verified against a small internal project, not battle-tested against every company's formatting quirks. The first time you run it against a new project, spot-check one or two files with a direct read of the raw JSON and confirm the counts roughly line up before trusting the rest of the sweep for that company. If they diverge, fall back to reading the files directly for that project instead of debugging the mismatch — never restrict detection to structural nodes only, even if a person tells you to skip *designing fixes* for AI nodes; detection is always in scope (Rule 7).

Patterns to search for (see `references/patterns.md` for the exact list, including both quote-style variants, and why each one matters):
...
- `$user.get("id")` inside CODE node `content` — this is the direct migrate target: replace it with `$user.get("phone")`, which works correctly on real WhatsApp (`references/patterns.md`).
- The `$user.get("phone") || $user.get("id")` (or equivalent try/catch) anti-pattern — flag it, but as **needs re-verification**, not as a confirmed bug (`references/patterns.md` has why).
- `{{$user.phone}}` **and `{{$user.phoneNumber}}`** mentions, especially inside AI_TASK prompts — `phoneNumber` is a documented alias returning the exact same value as `phone`, so count them together; a high combined count is a strong signal to prefer the native button design in Phase 9 even when it costs more to set up, because it avoids touching every prompt.
- A literal `$user.get("referenceId")` or `{{$user.referenceId}}` — **this is not a real property.** The workflow-scoped `$user` object only ever exposes `id`, `names`, `phone`, `phoneNumber` (per Jelou's own docs); `referenceId` is Jelou's own internal name for that same value, used only as a *field name in API request/response bodies*, never as a `$user` accessor inside a workflow. If found, flag it as **broken/dead code**, not as equivalent to `$user.get("id")`.
- A `referenceId` or `userReferenceId` field inside an `HTTP`/`TOOL`/`CODE` node's request body aimed at one of Jelou's own APIs (seen: Custom Channel "enviar interacción", Memoria "obtener-registro(s)") — the platform-identity field these endpoints expect, and it must carry `$user.get('id')`/`{{$user.id}}`. Classify like the `/users/{{$user.id}}/...` do-not-touch pattern below; if already swapped to `$user.phone`, that's a live bug (breaks for BSUID-unresolved contacts), not a correct migration.
...
- **`do-not-touch`**: the value feeds an internal Jelou platform endpoint — the signature is a URL shaped like `api.jelou.ai/v1/bots/{botId}/users/{id}/...` (seen: `/skill/...`, `/storedParams/legacy`), or a `referenceId`/`userReferenceId` body field on one of Jelou's own APIs (Custom Channel "enviar interacción", Memoria "obtener-registro(s)"). These need a stable, never-empty session identifier, which is exactly what `$user.id` provides and `$user.phone` does not (it can be empty for the very users this project protects). Explain this in the report so nobody "fixes" it later by mistake.
- **`do-not-touch` (recipient/destination fields):** `$user.id`/`{{$user.id}}` feeding the **recipient** of an outbound WhatsApp send — `userId` on `POST /v1/bots/{botId}/messages`, `destinations[]`/the CSV identifier column on `POST /v2/whatsapp/{botId}/hsm` and campaign sends, or a workflow `HSM` node's own recipient field. Jelou documents these as accepting **phone or BSUID interchangeably** — `$user.id` is the robust choice here, not a bug. **One documented exception:** an HSM/template send in the **AUTHENTICATION (OTP)** category requires a real phone number; a BSUID destination is silently skipped for that specific recipient. If you find an OTP/AUTHENTICATION send using `$user.id`/`{{$user.id}}` as its destination, don't classify it `do-not-touch` — flag it as its own warning in the report (BSUID-identified users silently never receive that OTP); there's no clean automatic fix since the phone still needs resolving first (that's exactly what Phase 9's guard does — point at it as the fix).
...
- A hand-rolled attempt already in place (the `$user.get("phone") || $user.get("id")` pattern, or similar). Report it as **needs re-verification on real WhatsApp** rather than a confirmed bug (`references/patterns.md`) — don't assert it's broken from reading the code alone.

**If an existing guard was found, before doing anything else with it, check it has a working `SUCCESS` output:** run `jelou workflow skill <skillId> --agent` and read `outputs[]` for an entry with `type: "SUCCESS"` (canonical name `resuelto`).
- **No `SUCCESS` output present** (`outputs: []`, or only non-`SUCCESS` entries): the guard can't be used as a real "wait for it, then continue" call — without a `SUCCESS` output, a `SKILL` node has nothing to wire an `output:*` edge to, so it can only run in the "legacy handoff" mode (fire-and-forget, the caller never waits). That defeats the entire point of a guard. This is a required fix, not cosmetic: `jelou workflow output add <skillId> --name resuelto --display-name "Telefono resuelto" --type SUCCESS`, plus an `END` node inside the guard's own workflow selecting it wherever `contact_info_request`'s `default` branch currently leads. Do this before wiring any caller to the guard.
- **A `SUCCESS` output already exists**: use its real `id` (never assume it — read it from this same call) when wiring `output:<id>:success` on every caller.
- **Ignore every other output the guard might have** (a leftover `no_resuelto`/`FAILED`, or anything else) **completely** — don't wire a caller against it, don't design around it, don't try to keep it in sync. `contact_info_request` has exactly one real way out once `expire` is left unwired (below), so the guard has exactly one output that matters: `SUCCESS`. A stray extra output is just unused metadata on the workflow, not something this skill fixes or touches.
...
2. Insert a `MEMORY` node right after its `START` templating `{{$user.phone}}` into a scratch variable, so you can confirm — for *this* company's specific provider and synthetic tester — that it resolves safely to `""` with no throw. **Never write `$user.get("phone")` into a CODE node as part of this check, and never use this probe (or any `jelou test` run) to draw any conclusion about `$user.get("phone")`'s behavior.** The synthetic tester's `$user` object has no `phone` key at all — real or fake, a probe of `$user.get("phone")` in CODE always throws there, so the result is uninformative about real customers and must not be reported as evidence either way (`references/patterns.md`). The template form (`{{$user.phone}}`) is the only phone-related thing this probe can meaningfully verify, because it behaves the same for the synthetic tester as for a real unresolved contact.
...
If an existing guard was found in Phase 7, extend/reuse its `skillId` instead of minting a new one. Wire every caller with `output:<id>:success` using the guard's *real* `SUCCESS` output `id` (from `jelou workflow skill`, per Phase 7) — never assume it matches the canonical shape's id. If Phase 7 had to add the missing `SUCCESS` output, use the `id` that command just returned. Ignore any other output the guard happens to have, same as Phase 7 — never wire a caller against it.
...
3. **Architecture**: one short paragraph + the routing-model evidence (the actual `jelou workflow evaluate` results, not just the conclusion), plus the list of workflows Phase 3 excluded — both non-WhatsApp channels (name + channel, or "none" if the project is WhatsApp-only) and stub workflows with nothing connected to their `START` (name + id, or "none" if all were live).
...
Execute Phase 9's plan file by file: create/extend the guard **as its own standalone workflow** (never as nodes inserted into the entry/target workflow itself — see Phase 9), wire it at the determined entry points via a `SKILL` node placed right after each target workflow's own `START`, migrate every `migrate`-classified hit (never `ambiguous` ones — those stay as reported warnings unless the person addressed them explicitly in their approval). For a `migrate`-classified hit inside a CODE node calling `$user.get("id")`, replace it directly with `$user.get("phone")` — confirmed working on real WhatsApp across multiple companies; no `MEMORY`/`$memory.get()` workaround needed, and don't try to verify this specific swap with `jelou test` (see `references/patterns.md` for why that probe can't confirm it). For every `jelou push`:
